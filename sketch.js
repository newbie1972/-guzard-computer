...
let weaponMode = 0; 
let weapons = [
  { name: "WAND", ammo: 6, magSize: 6, totalAmmo: 36, fireRate: 12, reloadTime: 90, bulletColor: [0, 200, 255] },
  { name: "SHOTGUN", ammo: 2, magSize: 2, totalAmmo: 6, fireRate: 50, reloadTime: 120, bulletColor: [255, 100, 0] },
  { name: "RIFLE", ammo: 1, magSize: 1, totalAmmo: 12, fireRate: 30, reloadTime: 110, bulletColor: [255, 0, 0] },
  { name: "SMG", ammo: 30, magSize: 30, totalAmmo: 90, fireRate: 5, reloadTime: 100, bulletColor: [0, 255, 0] }, 
  { name: "GUNBLADE", ammo: 10, magSize: 10, totalAmmo: 20, fireRate: 8, reloadTime: 90, bulletColor: [200, 150, 255] }
];
...
function drawEnemy() {
  push();
  fill(enemyFlash > 0 ? 255 : [200, 50, 50]); enemyFlash--;
...
function drawWizard(wx, wy) {
  ...
  else if (weaponMode === 2) { fill(40); rect(-10,-3,55,7); rect(-12,0,15,12); fill(80); rect(10,-6,15,4); } 
  else if (weaponMode === 3) { fill(30); rect(0,-4,30,9); fill(20); rect(10,0,6,15); } 
  else if (weaponMode === 4) { fill(50); rect(-5,-5,30,10); fill(180); triangle(25,-6,55,0,25,6); if(bladeExtension>5){fill(100,200,255,150); triangle(30,-8,30+bladeExtension,0,30,8);}} 
  else if (weaponMode === 5) { noStroke(); fill(255,100,0,200); for(let i=0;i<3;i++) circle(20+random(5),random(-5,5),15+sin(frameCount*0.2)*5); }
  pop();
}
